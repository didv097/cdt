...
<script>
  ...
  export default {
    ...
    methods: {
      getFiles () {
        this.loading = true
        axios.get('vessels/' + this.$route.params.id + '/files/drawings')
          .then(res => {
            this.files = res.data
            this.loading = false
          })
      ...
      uploadFile (event) {
        ...
        axios.post(
          'vessels/' + this.$route.params.id + '/files/drawings/upload',
          formData,
          {
            headers: {
      ...
      downloadFile (file) {
        axios.get('vessels/' + this.$route.params.id + '/files/drawings/' + file.name + '/download')
          .then(res => {
      ...
      deleteFile (file) {
        this.$confirm(`Please confirm that you would like to delete the following file: <b>${file.name}</b>`, {
          ...
          if (res) {
            axios.delete('vessels/' + this.$route.params.id + '/files/drawings/' + file.name + '/destroy')
              .then(res => {
                this.getFiles()
                this.showSnackBar(res.data.message, 'success')
              })
